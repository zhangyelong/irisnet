...
如上图所示，IRIS网络与Cosmos网络具有相同的拓扑结构。我们计划将IRIS枢纽连接到Cosmos枢纽作为其分区和二级枢纽之一。基于IRIS SDK开发的IRIS全节点（IRIS SDK将扩展Cosmos SDK的功能），旨在提供服务基础设施并提供与星际文件系统（“IPFS”）节点集成。

IRIS服务（又名“iServices”）打算通过一个完整的“链下服务”生命周期，从服务的定义、绑定（提供商注册）、调用、到治理（纠纷解决），连接区块链世界和传统的商业应用。 通过优化的IBC处理逻辑以支持服务语义，IRIS SDK旨在允许分布式商业服务在区块链互联网上可用。
